...
- schema-backed entity deepening through typed facet modules (`ownership`, `exposure`, `data_sensitivity`, and bucket support facets)
...
- entity support state surfaced as typed fields (`relationships`, `claim_count`, `supported_claim_count`, `conflicted_claim_count`, `evidence_count`, `observation_count`, `facets`, `posture`) instead of leaving asset context trapped in raw table rows or one-off reports
